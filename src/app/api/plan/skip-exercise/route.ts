...
export async function POST(req: Request) {
  ...
  try {
    // Phase 1: validate and commit the core skip atomically.
    ...
    const sessionRes = await client.query<SessionRow>(
      `select plan_session_id, block_id, date::text as date, performed_at::text as performed_at
       from plan_sessions
    ...
    // NOTE: We do NOT block on session.performed_at. performed_at is set as soon
    // as any set is logged in the session (not when all exercises are done), so
    // blocking skips on it prevents skipping an exercise after logging any other
    // exercise in the same session. exercise_already_started is the correct guard.

    const targetRes = await client.query<{ plan_exercise_id: string }>(
      `select plan_exercise_id from plan_exercises
       where plan_session_id = $1 and exercise_id = $2 and skipped_at is null`,
    ...
    const logsRes = await client.query(
      `select 1 from set_logs
       where user_id = $1 and session_id = $2 and exercise_id = $3 limit 1`,
    ...
    await client.query(
      `update plan_exercises set skipped_at = now() where plan_exercise_id = $1`,
      [targetRes.rows[0]?.plan_exercise_id]
    ).catch((error) => {
      if (!isMissingSkippedAtColumn(error)) throw error;
      return client.query(
        `delete from plan_exercises where plan_exercise_id = $1`,
        [targetRes.rows[0]?.plan_exercise_id]
      );
    });
    ...
    // Phase 2: secondary state sync. Non-fatal if it fails; skip is already committed.
    try {
      await client.query("BEGIN");
  ...
  return NextResponse.json({ ok: true, shifted: 0, dropped: 0 });
}
