...
export async function ensureWorkoutPlanForDate(
  ...
  if ((existingRes.rowCount ?? 0) > 0) {
    ...
    const sessionType = existingRes.rows[0]?.session_type ?? null;
    if (!sessionId) return null;

    // If session was generated by the new rolling scheduler (emphasis-based session_type),
    // reuse it only if it has exercises. Legacy block sessions (Mon/Tue/etc.) are always replaced.
    const isNewScheduler = sessionType && isSessionEmphasis(sessionType);
    if (isNewScheduler) {
      const activeExerciseCount = await countActivePlanExercises(client, sessionId);
      if (activeExerciseCount > 0) return sessionId;
    }

    // Delete legacy or empty session so the rolling scheduler can regenerate it
    await client.query(
